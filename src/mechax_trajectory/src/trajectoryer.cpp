...
//----------------------------------------------
    ...
    // 得到大yaw下的目标位置
    geometry_msgs::msg::PointStamped ps_yaw;
    ps_yaw.header.stamp = armor_time;
    ps_yaw.header.frame_id = "odom";
    ps_yaw.point.x = object_x;
    ps_yaw.point.y = object_y;
    ps_yaw.point.z = object_z;
    try
    {
        geometry_msgs::msg::PointStamped big_yaw_point;
        big_yaw_point.point = tf2_buffer_->transform(ps_yaw, "big_odom").point;
        float big_yaw_x = big_yaw_point.point.x;
        float big_yaw_y = big_yaw_point.point.y;
        big_yaw = atan2(big_yaw_y, big_yaw_x);
    }
    catch(const std::exception& e)
    {
        std::cerr << e.what() << '\n';
...
void Trajectoryer::targetCallback(const rm_msgs::msg::Target msg)
{
    ...
    if(is_tracking)
    {
        ...
        if(solve_trajectory() == 0)
        {
            ...
            float send_big_yaw = big_yaw * 57.3f;
