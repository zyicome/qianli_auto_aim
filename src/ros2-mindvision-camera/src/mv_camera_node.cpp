...
public:
  explicit MVCameraNode(const rclcpp::NodeOptions & options) : Node("mv_camera", options)
  {
    ...
    capture_thread_ = std::thread{[this]() -> void {
      ...
      auto mindvision_start = std::chrono::high_resolution_clock::now();
      auto mindvision_end = std::chrono::high_resolution_clock::now();
      int mindvison_fps = 0;
      ...
      while (rclcpp::ok()) {
        ...
        if (status == CAMERA_STATUS_SUCCESS) {
          ...
          mindvision_end = std::chrono::high_resolution_clock::now();

          std::chrono::duration<double> mindvision_diff = mindvision_end - mindvision_start;

          if(mindvision_diff.count() >= 1)
          {
            std::cout << "mindvision original fps: " << mindvison_fps << std::endl;
            mindvision_start = std::chrono::high_resolution_clock::now();
            mindvison_fps = 0;
          }
