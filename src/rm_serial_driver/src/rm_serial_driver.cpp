              ...
              data_buffer.push_back(header[0]);
                  ...
                  if (data_buffer.size() == sizeof(ReceivePacket) + 1) {
                    ...
                    if (header[0] == 0xAA)
                    {
                      ...
                      //timestamp_offset_ = this->get_parameter("timestamp_offset").as_double();
                      t_yaw.header.stamp = this->now() - rclcpp::Duration::from_seconds(timestamp_offset_);
                      t_yaw.header.frame_id = "odom";
                      t_yaw.child_frame_id = "yaw_gimbal_link";
                      tf2::Quaternion q_yaw;
                      q_yaw.setRPY(0, 0, packet.yaw / 57.3f);
                      t_yaw.transform.rotation = tf2::toMsg(q_yaw);
                      tf_broadcaster_->sendTransform(t_yaw);

                      geometry_msgs::msg::TransformStamped t_pitch;
                      t_pitch.header.stamp = this->now() - rclcpp::Duration::from_seconds(timestamp_offset_);
                      t_pitch.header.frame_id = "pitch_gimbal_link";
                      t_pitch.child_frame_id = "gimbal_link";
                      tf2::Quaternion q_pitch;
                      q_pitch.setRPY(0, packet.pitch / 57.3f, 0);
                      t_pitch.transform.rotation = tf2::toMsg(q_pitch);
                      tf_broadcaster_->sendTransform(t_pitch);
