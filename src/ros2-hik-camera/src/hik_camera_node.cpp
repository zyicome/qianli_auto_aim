...
public:
  explicit HikCameraNode(const rclcpp::NodeOptions & options) : Node("hik_camera", options)
  {
    ...
    capture_thread_ = std::thread{[this]() -> void {
      ...
      auto hik_start = std::chrono::high_resolution_clock::now();
      auto hik_end = std::chrono::high_resolution_clock::now();
      int hik_fps = 0;
      ...
      while (rclcpp::ok()) {
        ...
        if (MV_OK == nRet) {
          ...
          hik_end = std::chrono::high_resolution_clock::now();

          std::chrono::duration<double> hik_diff = hik_end - hik_start;

          if(hik_diff.count() >= 1)
          {
            std::cout << "hik original fps: " << hik_fps << std::endl;
            hik_start = std::chrono::high_resolution_clock::now();
            hik_fps = 0;
          }
